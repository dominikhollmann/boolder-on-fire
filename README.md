...
Waldbrand-Sperrzonen für Fontainebleau, dargestellt auf einer [Boolder](https://www.boolder.com)-artigen
Boulder-Karte.

Fontainebleau hat in Teilen Waldbrand-bedingte Zutrittsverbote. Diese Sperrzonen werden
...
in Fontainebleau, zeigt diese Sperrzonen aber nicht an. Dieses Repo ist ein Prototyp, der die
roten Sperrflächen zusätzlich zu den Boolder-Boulderpunkten auf einer Karte anzeigt und die
Daten automatisch aktuell hält — als Vorlage für eine mögliche echte Erweiterung von Boolder
(siehe [`upstream-patch/`](upstream-patch/)).
...
js/map.js                       # Kartenaufbau: Boolder-Style + Boulderpunkte + closures-Layer (rot)
...
scripts/sync-closures.mjs       # Node-Skript: holt aktuelle Zonen von uMap, schreibt data/fire-closures.geojson
...
`scripts/sync-closures.mjs` holt die aktuellen Sperrzonen von der uMap-Karte, reduziert sie
auf die für die Anzeige nötigen Felder und schreibt `data/fire-closures.geojson`. Die
